...
import { simulateCobolExecution } from '../services/geminiService';
import { AIChatResponse } from '../types';

interface CobolRendererProps {
  cobolCode: string;
}
...
const CobolRenderer: React.FC<CobolRendererProps> = ({ cobolCode }) => {
  const [output, setOutput] = useState<string>('');
  const [isExecuting, setIsExecuting] = useState(false);
  const [isFullscreen, setIsFullscreen] = useState(false);
  const [editableCobolCode, setEditableCobolCode] = useState(cobolCode);
  ...
  const runSimulation = async () => {
    ...
    setOutput('');
    try {
      const response: AIChatResponse = await simulateCobolExecution(editableCobolCode);
  ...
  return (
    <div ref={rendererRef} className={`w-full bg-[var(--color-bg-primary)] rounded-lg border border-[var(--color-border)] shadow-inner flex flex-col ${isFullscreen ? 'h-full' : 'h-96'}`}>
      ...
      <div className="flex items-center justify-between p-1 px-3 bg-[var(--color-bg-secondary)] border-b border-[var(--color-border)] flex-shrink-0">
        <span className="text-xs font-semibold text-[var(--color-text-secondary)]">Editor COBOL</span>
        <div className="flex items-center gap-2">
            ...
            >
              ...
              <span>{isExecuting ? 'Compilando...' : 'Compilar & Executar'}</span>
            </button>
            <button
      ...
      <textarea
        ...
        value={editableCobolCode}
        onChange={(e) => setEditableCobolCode(e.target.value)}
        spellCheck="false"
        disabled={isExecuting}
        aria-label="Editor de código COBOL"
      ></textarea>
